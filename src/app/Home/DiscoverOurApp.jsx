...
export default function DiscoverOurApp() {
  ...
  return (
    <div className="h-fit w-full relative bg-[#12131D] text-[#D2C2E6] py-[5vw] px-[15vw] max-sm:px-[5vw] max-sm:py-[10vw]">
      ...
      <div className=" w-full flex items-center max-sm:mt-[2vw] max-sm:mb-[5vw] max-sm:justify-center justify-end">
        <p
          id="discoverApp"
          className="text-[1.3vw] max-sm:text-[4vw] max-sm:text-center max-sm:w-[80%] font-bold leading-[1.2] w-[42%]"
        >
      ...
      <div className="h-[100vh] max-sm:h-[60vh] cards-container w-full flex items-center relative justify-center py-[5vw]">
      
        {cards.map((image, index) => (
          <div
            key={index}
            className="w-[16vw] max-sm:w-[40vw] h-auto absolute z-[100] discover-app-card"
            style={{
              zIndex: image.zIndex,
      ...
      <div className="w-full h-fit py-[3vw]  flex items-start justify-center flex-col">
        ...
        <div className="h-[35vh] max-sm:h-fit  max-sm:mt-[8vw]  w-full flex flex-col flex-wrap max-sm:gap-[2.5vw] max-sm:px-[5vw] gap-[.5vw]">
          {productSpecs.map((spec, index) => (
            <div
              key={index}
              className="w-[50%] max-sm:w-[100%] h-fit flex items-center gap-4vw max-sm:gap-[10vw]"
            >
              <p className="text-[.85vw] max-sm:text-[3.5vw] opacity-50 w-[30%] font-semibold">
                {spec.category}
              </p>
              <p className="text-[.9vw] max-sm:text-[3.5vw] w-[52%] text-white">{spec.value}</p>
            </div>
          ))}
        </div>
      </div>
